 ...
 * http://implix.com
 * Modified to work with monerowp-rpc wallet by Serhack and cryptochangements
 */
class Monero_Library
{
    ...
    protected function & getResponse(&$pRequest)
    {
        ...
        if (0 < curl_errno($ch)) {
           echo '[ERROR] Failed to connect to monerowp-wallet-rpc at ' . $this->host . ' port '. $this->port .'</br>';
        }
    ...
    /* 
     * The following functions can all be called to interact with the monerowp rpc wallet
...
class NodeTools
{
    public function get_last_block_height()
    {
        ...
        curl_setopt_array($curl, array(
                                       ...
                                       CURLOPT_URL => 'https://stagenet.xmrchain.net/api/networkinfo',
                                       ));
        $resp = curl_exec($curl);
        curl_close($curl);
    ...
    public function get_txs_from_block($height)
    {
        ...
        curl_setopt_array($curl, array(
                                       CURLOPT_RETURNTRANSFER => 1,
                                       CURLOPT_URL => 'https://stagenet.xmrchain.net/api/search/' . $height,
                                       ));
        $resp = curl_exec($curl);
        curl_close($curl);
    ...
    public function check_tx($tx_hash, $address, $viewKey)
    {
        ...
        curl_setopt_array($curl, array(
                                       CURLOPT_RETURNTRANSFER => 1,
                                       CURLOPT_URL => 'https://stagenet.xmrchain.net/api/outputs?txhash=' .$tx_hash . '&address='. $address . '&viewkey='. $viewKey .'&txprove=0',
                                       ));
        $resp = curl_exec($curl);
        curl_close($curl);
        $array = json_decode($resp, true);
    ...
    function get_mempool_txs()
    {
        ...
        curl_setopt_array($curl, array(
                                       CURLOPT_RETURNTRANSFER => 1,
                                       CURLOPT_URL => 'https://stagenet.xmrchain.net/api/mempool',
                                       ));
        $resp = curl_exec($curl);
        curl_close($curl);
        $array = json_decode($resp, true);
